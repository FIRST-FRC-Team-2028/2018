...
//	 AnalogInput camera_x;
	 ...
	 private Lift lift;
	 private Drive drive;
	 double P = Parameters.CAMERA_P;
	 public double  Auto_Selection = 0;
	 int i = 0;
	 double testcount;
	boolean leftswitch;
	boolean leftscale;
...
//	 public static WPI_TalonSRX Test_Talon;
	...
	public void robotInit() {
		 lift = new Lift();
		 drive = new Drive();
		 pidController = new PIDController(Parameters.CONTROLLER_P , Parameters.CONTROLLER_I , Parameters.CONTROLLER_D,
				 Parameters.CONTROLLER_F , gyro , drive);
		 pidController.setOutputRange(-1000, 1000);
		 cameraController = new PIDController(Parameters.CAMERA_P, Parameters.CAMERA_I, Parameters.CAMERA_D, 
				 Parameters.CAMERA_F, pixycamera, drive);
...
//	  
	...
	public void autonomousPeriodic() {
		...
		SmartDashboard.putBoolean("Scheduler run", true);
...
//		Left_Drive_Master.enableVoltageCompensation(true);
	...
	public void teleopPeriodic() {
		...
		SmartDashboard.putNumber("P VALUE", Parameters.CAMERA_P);
	...
	if (Math.abs(Controller.getRawAxis(1)) > .08)
	   {
	...
	if (Math.abs(Controller.getRawAxis(5)) > .08)
	   {
...
//		double rotatespeed = 400;
		...
		} else if ((Controller.getRawButton(5) && !pidController.isEnabled()) || (Controller.getRawButton(5) && !cameraController.isEnabled())) 
		{
			...
			drive.speedControl(leftStickSpeed);
		}
		...
		if(LaunchPad.getRawButton(7))
		{
			...
			drive.pidRotate();
		}else if(cameraController.isEnabled()){
